...
eval(fs.readFileSync(path.join(__dirname, '..', 'data', 'matchum.js'), 'utf8'));   // 카훗 원본 — raw 없음
var fail = 0;
function err(msg) { console.error('FAIL', msg); fail = 1; }

['hunmin', 'yongbi', 'sohak', 'matchum'].forEach(function (qid) {
  var quiz = global.window.QUIZZES[qid];
  // 학생정보 3문항 중 short는 '이름' 하나뿐(학교/학년은 radio) → slice(1). 카훗 원본은 raw 없음
  var raw = global.window.RAW && global.window.RAW[qid]
    ? global.window.RAW[qid].items.filter(function (it) { return it.type === 'short' || it.type === 'check'; }).slice(1)
    : null;
  if (raw && quiz.questions.length !== raw.length) err(qid + ': 문항 수 불일치 data=' + quiz.questions.length + ' raw=' + raw.length);
  var ids = {};
  quiz.questions.forEach(function (q, i) {
    if (ids[q.id]) err(qid + ': id 중복 ' + q.id);
    ids[q.id] = 1;
    if (!q.prompt) err(q.id + ': prompt 없음');
    if (!Array.isArray(q.choices) || q.choices.length < 2) err(q.id + ': 선지 2개 미만');
